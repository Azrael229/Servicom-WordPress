...
class Form_Server {
	...
	public function submit_form( $request ) {
		...
		if ( ! $this->has_requiered_data( $data ) ) {
			$return['error']   = __( 'Invalid request!', 'otter-blocks', 'themeisle-companion' );
			$return['reasons'] = __( 'Essential data is missing!', 'otter-blocks', 'themeisle-companion' );
			return $return;
		}
		...
		if ( isset( $data['token'] ) ) {
			...
			if ( false == $result['success'] ) {
				$return['error'] = __( 'The reCaptha was invalid!', 'otter-blocks', 'themeisle-companion' );
	...
	private function send_email( $data ) {
		...
		$email_subject = ( isset( $data['emailSubject'] ) ? $data['emailSubject'] : ( __( 'A new form submission on ', 'otter-blocks', 'themeisle-companion' ) . get_bloginfo( 'name' ) ) );
	...
	private function prepare_body( $data ) {
		...
		<head>
			...
			<title><?php esc_html__( 'Mail From: ', 'otter-blocks', 'themeisle-companion' ) . sanitize_email( get_site_option( 'admin_email' ) ); ?></title>
		</head>
		<body>
		<table>
			<thead>
			<tr>
				<th colspan="2">
					<h3>
						<?php esc_html_e( 'Content Form submission from ', 'otter-blocks', 'themeisle-companion' ); ?>
			...
			<tr>
				<td>
					<hr/>
					<?php esc_html_e( 'You received this email because your email address is set in the content form settings on ', 'otter-blocks', 'themeisle-companion' ); ?>
					<a href="<?php echo esc_url( get_site_url() ); ?>"><?php bloginfo( 'name' ); ?></a>
				</td>
			</tr>
			</tfoot>
	...
	public function get_integration_data( $request ) {
		...
		$return['error'] = __( 'Invalid request! Provider is missing.', 'otter-blocks', 'themeisle-companion' );
		return rest_ensure_response( $return );
	}
	...
	private function check_form_conditions( $data ) {
		...
		if ( $has_captcha && ( ! isset( $data['token'] ) || '' === $data['token'] ) ) {
			$reasons += array(
				__( 'Token is missing!', 'otter-blocks', 'themeisle-companion' ),
			);
		}
		...
		if ( ! $has_creditentials && $integration['provider'] ) {
			$reasons += array(
				__( 'Provider settings are missing!', 'otter-blocks', 'themeisle-companion' ),
			);
		}
		return $reasons;
	}
